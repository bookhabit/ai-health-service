...
export async function POST(request:Request) {
    ...
    // define work out message prompt
      ...
      //   },
      // ];
      let message = '오늘 아침 운동 챌린지 시작 10분 전입니다! 스트레칭을 진행해주세요.'
      ...
      if (userThread) {
        ...
        if (cp.sendNotifications) {
          ...
          if(correspondingUserMeta){
            if(cp.challengeId === "EASY"){
              message += ' 오늘의 챌린지는 푸쉬업 50개,스쿼트 50개입니다.'
            }else if(cp.challengeId === "MEDIUM"){
              message += ' 오늘의 챌린지는 푸쉬업 100개,스쿼트 100개입니다.'
            }else if(cp.challengeId === "HARD"){
              message += ' 오늘의 챌린지는 푸쉬업 200개,스쿼트 200개입니다.'
            }
